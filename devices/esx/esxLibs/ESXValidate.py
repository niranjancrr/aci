...
class ESXValidate():
    ...
    def __init__(self):
        self.log.info("Intializing API's for ESXi show command Validations")
        ...
        while counter:
            try:
                ...
                if re.search('{}\.\d+\.\d+\.\d+'.format(subnet),self.contents) == None:
                    raise
                flag = True
                self.log.info("ESX {} was able to aquire DHCP IP {} on {} interface in {} secs" .format(self.ip,ip,vmk,elapsed))
                return flag
            except:
                self.log.error("ESX {} did not get DHCP IP on {} interface in {} secs" .format(self.ip,vmk,elapsed))
            ...
            try:
                if override:
                    if self.get_opflex_status(override):
                        self.log.info("Opflex came up on ESX {} in {} secs" .format(self.ip,elapsed))
                        return True
                    else:
                        raise
                else:
                ...
                self.log.error("Opflex did not come up on ESX {} in {} secs" .format(self.ip,elapsed))
                self.log.info("Sleeping for 10 secs")
                time.sleep(10)
                elapsed += 10
                counter -= 10
    ...
    def validate_port_state(self):
        '''
            Make sure all ports are in FWD state on the current ESX host
    ...
    def get_vem_version_info(self):
        ...
        versionDict['vmware_version'] = re.search('System Version: VMware ESXi\s+(\d+\.\d+\.\d+)',op).group(1)
